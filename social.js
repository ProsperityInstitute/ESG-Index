...
function currentRows() {
  ...
  rows.sort((a, b) => {
    const av = sortCol === 'rank'
      ? a.rank
      : sortCol === 'tier'
        ? tierLabel(a.Social_Score)
        : a[sortCol];

    const bv = sortCol === 'rank'
      ? b.rank
      : sortCol === 'tier'
        ? tierLabel(b.Social_Score)
        : b[sortCol];
...
function renderTable() {
  ...
  document.getElementById('tableBody').innerHTML = rows.map(d => {
    ...
    return `<tr class="ranking-row" onclick="window.location.href='profile.html?ticker=${encodeURIComponent(d.Ticker)}'">
      ...
      <td><div class="metric-cell">${metricMarkup(d.DEI_Targets_Representation)}</div></td>
      <td><div class="metric-cell">${metricMarkup(d.DEI_Programmes_Memberships)}</div></td>
      <td><div class="metric-cell">${metricMarkup(d.Social_Incentives)}</div></td>
      <td><div class="metric-cell">${metricMarkup(d.Social_Score, true)}</div></td>
      <td><div class="tier-cell"><span class="tier-pill tier-${tier}">${tierLabel(d.Social_Score)}</span></div></td>
    </tr>`;
  }).join('');
}
  ...
  document.querySelectorAll('.table-header [data-sort]').forEach(th => {
    th.addEventListener('click', () => {
      ...
      if (sortCol === col) {
        ...
        sortCol = col;
        sortAsc = col === 'Social_Score' || col === 'rank';
      }
      renderTable();
    });
  });
