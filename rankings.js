...
function renderTable() {
  ...
  document.getElementById('tableBody').innerHTML = rows.map(d => {
    ...
    return `<tr class="ranking-row" onclick="window.location.href='profile.html?ticker=${encodeURIComponent(d.Ticker)}'">
      ...
      <td><div class="metric-cell">${metricMarkup(d.Climate_Targets)}</div></td>
      <td><div class="metric-cell">${metricMarkup(d.Investment_Transition)}</div></td>
      <td><div class="metric-cell">${metricMarkup(d.Climate_Reporting)}</div></td>
      <td><div class="metric-cell">${metricMarkup(d.Environment_Score, true)}</div></td>
      <td><div class="tier-cell"><span class="tier-pill tier-${tier}">${tierLabel(d.Environment_Score)}</span></div></td>
    </tr>`;
  }).join('');
}
